...
var steps = 0
def solve(i0: Int, j0: Int, cells: Array[Array[Int]]): Boolean = boundary:
  steps += 1
  var i = i0
  var j = j0
  if i == 9 then
    j += 1;
    if j == 9 then break(true)
    else i = 0

  // skip filled cells
  if cells(i)(j) != 0 then break(solve(i + 1, j, cells))

  for value <- Range.inclusive(1, 9) do
    cells(i)(j) = value
    if isValidSudoku(cells) && solve(i + 1, j, cells) then break(true)
