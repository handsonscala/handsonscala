...
def main() =
  ...
  assert(
    evaluate(fastparse.parse("\"hello\" + \"world\"", Parser.expr(using _)).get.value, Map.empty) ==
  ...
  assert(
    evaluate(fastparse.parse(input2, Parser.expr(using _)).get.value, Map.empty) ==
    Value.Str("Hello Hello ")
  )
  val input3 = """local x = "Hello "; local y = "world"; x + y"""

  assert(
    evaluate(fastparse.parse(input3, Parser.expr(using _)).get.value, Map.empty) ==
  ...
  assert(
    evaluate(fastparse.parse(input4, Parser.expr(using _)).get.value, Map.empty) ==
    Value.Str("1231")
  )
  ...
  assert(
    evaluate(fastparse.parse(input5, Parser.expr(using _)).get.value, Map.empty) ==
    Value.Str("hello world")
  )
